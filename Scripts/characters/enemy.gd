...
@export var enemy_repel_force := 150.0  # Force to push away from other enemies
@export var enemy_detection_radius := 40.0  # Distance to detect other enemies
...
func _physics_process(delta: float) -> void:
	...
	# Handle enemy repulsion first (highest priority)
	handle_enemy_repulsion()
	
	# Only do normal behavior if not being strongly repelled
	...
	# Apply repel velocity on top of normal movement
	...
	repel_velocity = repel_velocity * 0.8
...
func handle_enemy_repulsion():
	...
	# Check for nearby enemies using collision detection and area scanning
	var space_state = get_world_2d().direct_space_state
	var query = PhysicsPointQueryParameters2D.new()
	query.position = global_position
	query.collision_mask = collision_mask  # Use same collision mask as this enemy
	query.exclude = [self]  # Don't detect self
	
	# Also check for enemies in a radius around this enemy
	...
	# Check slide collisions for immediate contact
	for i in get_slide_collision_count():
		...
		if other and other != self and other.is_in_group("enemy"):
			...
			# Strong immediate repulsion for direct contact
	...
	if enemy_count > 0:
		...
		# Limit maximum repel velocity to prevent excessive speeds
		...
		# If being repelled strongly, temporarily override other behaviors
